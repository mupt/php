...
echo '<fieldset><legend>Delete a brand</legend>';
$page_title = 'Delete a brand';
...
if (isset($_POST['submitted'])) {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
	   ...
	   if (mysqli_num_rows($result) == 1) { // Valid dir. ID, show the result.
		...
		$brand_name_del=$row[1];
		$brand_origin_del=$row[2];
		...
		<p>The brand name <strong>'.$brand_name_del. ' and country ' .$brand_origin_del.'</strong> has been deleted.</p><p><br /><br /></p>';	
		...
		<p class="error">This page has been accessed in error.</p><p><br /><br /></p>';
			...
			<p class="error">This brand could not be deleted due to a system error.</p>'; // Public message.
			echo '<p>' . mysqli_error($dbc) . '<br /><br />Query: ' . $query . '</p>'; // Debugging message.
		}
	
	} else { // Wasn't sure about deleting the movie.
		echo '<h1 id="mainhead">Delete a brand</h1>';
	...
	if (mysqli_num_rows($result) == 1) { // Valid movie ID, show the result.
		...
		// Create the result page.
  echo'
		<p>The brand name <strong>'.$brand_name_del. ' and country ' .$brand_origin_del.'</strong> has NOT been deleted.</p><p><br /><br /></p>';	
	...
	if (mysqli_num_rows($result) == 1) { // Valid dir. ID, show the form.
		...
		// Create the form.
		echo '
	<form action="del_brand.php" method="post">
	<h3>Brand Name: ' . $row[0] . '</h3>
	<h3>Brand Origin: ' . $row[1] . '</h3>
	...
	<input type="hidden" name="id" value="' . $id . '" />
		...
		echo '<a href="edit_brand.php?id=' . $id . '">Edit</a>';
