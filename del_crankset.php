...
if (isset($_POST['submitted'])) {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
		...
		// Make the query.
	   $query = "select * from cr_brand where cr_brand_id=$id";		
	   ...
	   if (mysqli_num_rows($result) == 1) { // Valid dir. ID, show the result.
		...
		$cr_brand_name_del=$row[1];
		
		$query = "DELETE FROM cr_brand WHERE cr_brand_id=$id";		
		...
		<p>The crankset brand name <strong>'.$cr_brand_name_del. '</strong> has been deleted.</p><p><br /><br /></p>';	
	...
	if (mysqli_num_rows($result) == 1) { // Valid movie ID, show the result.
		...
		// Create the result page.
  echo'
		<p>The crankset brand name <strong>'.$cr_brand_name_del. '</strong> has NOT been deleted.</p><p><br /><br /></p>';	
	...
	if (mysqli_num_rows($result) == 1) { // Valid dir. ID, show the form.
		...
		// Create the form.
		echo '
	<form action="del_crankset.php" method="post">
