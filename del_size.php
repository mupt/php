...
// This page deletes a size
// This page is accessed through view_brand.php.
echo '<fieldset><legend>Delete a size</legend>';
$page_title = 'Delete a size';

// Check for a valid dir. ID, through GET or POST.
if ( (isset($_GET['id'])) && (is_numeric($_GET['id'])) ) { // Accessed through del_size.php
	$id = $_GET['id'];
...
if (isset($_POST['submitted'])) {
	...
	if ($_POST['sure'] == 'Yes') { // Delete the record.
	   ...
	   if (mysqli_num_rows($result) == 1) { // Valid dir. ID, show the result.
		...
		<p>The size <strong>'.$size_name_del. '</strong> has been deleted.</p><p><br /><br /></p>';	
		...
		<p class="error">This page has been accessed in error.</p><p><br /><br /></p>';
			...
			<p class="error">This size could not be deleted due to a system error.</p>'; // Public message.
			echo '<p>' . mysqli_error($dbc) . '<br /><br />Query: ' . $query . '</p>'; // Debugging message.
		}
	
	} else { // Wasn't sure about deleting the movie.
		echo '<h1 id="mainhead">Delete a size</h1>';
	...
	if (mysqli_num_rows($result) == 1) { // Valid movie ID, show the result.
		...
		// Create the result page.
  echo'
		<p>The size <strong>'.$size_name_del. '</strong> has NOT been deleted.</p><p><br /><br /></p>';	
	...
	if (mysqli_num_rows($result) == 1) { // Valid dir. ID, show the form.
		...
		// Create the form.
		echo '
	<form action="del_size.php" method="post">
	<h3>Size in Centimeters ' . $row[0] . '</h3>
	...
	<input type="hidden" name="id" value="' . $id . '" />
		...
		echo '<a href="edit_size.php?id=' . $id . '">Edit</a>';
