...
echo "<fieldset><legend>Edit selected kinds</legend>";

$page_title = 'Edit kinds';
...
if (isset($_POST['submitted'])) {
	...
	if (empty($_POST['kind_name'])) {
		$errors[] = 'You forgot to enter the kind name.';
	} else {
	...
	if(empty($_POST['kind_description'])) {
		...
		$kind_description=$_POST['kind_description'];
	}
	...
	if (empty($errors)) { // If everything's OK.
			...
			// Make the query.
			$query= "UPDATE kind SET kind_name='$kind_name', kind_description='$kind_description' WHERE kind_id = $id";
			$result = @mysqli_query ($dbc, $query); // Run the query.
			if ((mysqli_affected_rows($dbc) == 1) || (mysqli_affected_rows($dbc) == 0)) { // If it ran OK.
			
				// Print a message.
				echo '<h1 id="mainhead"></h1>
				<p>The kind has been edited.</p><p><br /><br /></p>';	
							
			} else { // If it did not run OK.
				echo '<h1 id="mainhead">System Error</h1>
				<p class="error">The kind could not be edited due to a system error. We apologize for any inconvenience. Please don\'t blame yourself. </p>'; // Public message.
...
<form action="edit_kind.php" method="post">
<p>Kind of Bicycle:<input type="text" name="kind_name" size="25" maxlength="25" value="' . $row[0] . '" /></p>
<p>Description:<input type="text" name="kind_description" size="25" maxlength="25" value="' .$row[1] . '" /></p>
