<?php # view_model_by_id.php

$page_title = 'View Models by Brand';
echo '<fieldset>';
// Page header.
echo '<h1 style="text-align:center"id="mainhead">Models of selected brand:</h1>';
echo '<p>&nbsp;</p>';
...
if (isset($_GET['np'])) { // Already been determined.
	...
	$query = "SELECT count(distinct model_name) FROM model where brand_id=$id";
...
if ($num_pages > 1) {
	...
	if ($current_page != 1) {
		echo '<a href="view_model_by_id.php?'.'id='.$id.'&s=0&np=' . $num_pages . '&sort=' . $sort .'">First</a> ';
		echo '<a href="view_model_by_id.php?'.'id='.$id.'&s=' . ($start - $display) . '&np=' . $num_pages . '&sort=' . $sort .'">Previous</a> ';
	}
	
	// Make all the numbered pages.
	for ($i = 1; $i <= $num_pages; $i++) {
		if ($i != $current_page) {
			echo '<a href="view_model_by_id.php?'.'id='.$id.'&s=' . (($display * ($i - 1))) . '&np=' . $num_pages . '&sort=' . $sort .'">' . $i . '</a> ';
		} else {
			echo $i . ' ';
		}
	}
	...
	if ($current_page != $num_pages) {
		echo '<a href="view_model_by_id.php?'.'id='.$id.'&s=' . ($start + $display) . '&np=' . $num_pages . '&sort=' . $sort .'">Next</a> ';
		echo '<a href="view_model_by_id.php?'.'id='.$id.'&s=' . (($num_pages-1) * $display) . '&np=' . $num_pages . '&sort=' . $sort .'">Last</a>';
